...
const sources = [...await files('src'), ...await files('prompts'), 'action.yml', 'package-lock.json', 'scripts/build.ts', 'scripts/gemini-smoke.ts'];
...
for (const [entry, output] of [['src/entry.ts', 'dist/index.js'], ['scripts/gemini-smoke.ts', 'dist/gemini-smoke.cjs']]) await build({ entryPoints: [new URL(entry!, root).pathname], outfile: new URL(output!, root).pathname,
